...
export default function Updateinvoiceform(props){
    let history = useHistory();
    let extra = props.data.bill_details.items.slice(1,props.data.bill_details.items.length);
    let [form, setForm] = useState(
        extra.map((ele)=>{
            return {
                description:ele.item_desc,
                quantity:ele.qty,
                rates:ele.rate,
                amnt:ele.amount,
                errors:{
    ...
    let formik = useFormik({
        initialValues: {
          invno:props.data.invoice_number,invdate:props.data.created_date,duedate:props.data.due_date,bcompany: props.data.biller_details.company_name,
          ccompany:props.data.client_details.company_name,bname:props.data.biller_details.biller_name,cemail:props.data.client_details.email,baddl1:props.data.biller_details.address_line1,caddl1:props.data.client_details.address_line1,baddl2:props.data.biller_details.address_line2,
          caddl2:props.data.client_details.address_line2,bcity:props.data.biller_details.city, ccity:props.data.client_details.city,bstate:props.data.biller_details.state,cstate:props.data.client_details.state,bcountry:props.data.biller_details.country, ccountry:props.data.client_details.country, notes:props.data.notes, 
          terms:props.data.terms,desc:props.data.bill_details.items[0].item_desc,qty:props.data.bill_details.items[0].qty,rate:props.data.bill_details.items[0].rate,amount:props.data.bill_details.items[0].amount,subtot:0,taxp:props.data.bill_details.tax_percent,taxa:0,total:0
        },
        validate: (values) => {
          let errors = {};
          if (!values.invno) {
    ...
    return <>
    <h1>this is update invoice form {props.data._id}</h1>
                        ...
                        </div>
                         {
                            form.map((item, index) => (
                                <div className="row pt-4" key={`item-${index}`}>
                                  <div className="col-md-5">
                                    <input type="text" className={item.errors.description? "form-control is-invalid": "form-control"} id="description" name="description" placeholder="Item description"
